...
def tokenize_delimiter_string(tokenizer, delimiter_str: str, device: torch.device):
    """
    Tokenize a delimiter string for substring matching mode.

    Also computes merged-boundary variants: some tokenizers fuse the delimiter's
    last character with a following newline into a single token
    (e.g. Qwen2.5: ">" (29) + "\\n" → ">\\n" (397), ">" + "\\n\\n" → ">\\n\\n" (1339)).
    The variants have the same length as the base sequence so delimiter_length is
    unchanged and delimiter_end arithmetic stays correct.
    ...
    Returns:
        List[Tensor]: [base_ids, variant1_ids, ...] — base first, then any merged-boundary
                      variants (empty list tail if no merging occurs for this tokenizer).
                      All tensors have the same length (subseq_len,).
    """
    base_ids = tokenizer.encode(delimiter_str, add_special_tokens=False)
    base_t = torch.tensor(base_ids, dtype=torch.long, device=device)
    base_len = len(base_ids)

    all_delimiters = [base_t]
    for suffix in ['\n', '\n\n']:
        merged_ids = tokenizer.encode(delimiter_str + suffix, add_special_tokens=False)
        # A merge occurred when the suffix was absorbed into the last token
        # (same total length as base means the suffix character merged in)
        if len(merged_ids) == base_len and merged_ids != base_ids:
            all_delimiters.append(torch.tensor(merged_ids, dtype=torch.long, device=device))
...
def find_special_token_positions(
    ...
    end_think_delimiter_ids=None,  # Optional[List[torch.Tensor]] from tokenize_delimiter_string
    ...
    if end_think_token_id is not None:
        ...
        # end_think_delimiter_ids is a List[Tensor] from tokenize_delimiter_string:
        #   [base, variant1, variant2, ...] — base first, then merged-boundary variants.
        # e.g. Qwen2.5 "</think>": [[522,26865,29], [522,26865,397], [522,26865,1339]]
        #   ">" (29) + "\n" → ">\n" (397), ">" + "\n\n" → ">\n\n" (1339)
        # All entries have the same length so delimiter_length arithmetic is unchanged.
        end_think_positions = torch.full((batch_size,), -1, dtype=torch.long, device=device)
        delimiter_length = end_think_delimiter_ids[0].shape[0]

        for i in range(batch_size):
            # IMPORTANT: search only within the response portion.
            # The prompt can contain "</think>" as part of format instructions
            # (e.g. "response format: <think>reasoning</think> Final answer: X").
            # Searching the full sequence would find the prompt's occurrence when
            # the response's occurrence has a different tokenization at the boundary.
            response_seq = input_ids[i, response_start:]

            pos = -1
            for delim in end_think_delimiter_ids:
                p = _find_subsequence_in_sequence(response_seq, delim)
                if p >= 0:
                    pos = p
                    break  # base match takes priority; variants are fallbacks only

            end_think_positions[i] = (response_start + pos) if pos >= 0 else -1
