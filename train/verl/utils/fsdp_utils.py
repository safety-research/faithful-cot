    ...
    if min_num_params > 0:
        ...
        for layer_class in fsdp_transformer_layer_cls_to_wrap:
            ...
            if transformer_cls is None:
                not_found_layers.append(layer_class)
            else:
                transformer_cls_to_wrap.add(transformer_cls)

        # Raise error only if NO valid transformer layers were found
        if len(transformer_cls_to_wrap) == 0:
            raise Exception(
                f"Could not find any transformer layer classes to wrap in the model. "
                f"Searched for: {fsdp_transformer_layer_cls_to_wrap}. "
                f"Please check your model architecture or FSDP wrap policy configuration."
            )

        # Log warning if some layers weren't found (common for multimodal models loaded as text-only)
        if not_found_layers:
            print(
                f"[FSDP Wrap Policy] Warning: Could not find {len(not_found_layers)} layer class(es): {not_found_layers}. "
                f"This is expected for models like Gemma3 when loading text-only variants. "
                f"Found {len(transformer_cls_to_wrap)} valid layer(s): {[cls.__name__ for cls in transformer_cls_to_wrap]}"
            )
