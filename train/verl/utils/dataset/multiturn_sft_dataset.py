...
class MultiTurnSFTDataset(Dataset):
    ...
    def _process_single_message(
        ...
        # Try to apply chat template with single message
        dummy_prefix_length = 0
        try:
            ...
            # If it fails (likely due to role alternation), add a dummy user message
            if "must alternate" in str(e).lower() or "conversation roles" in str(e).lower():
                dummy_user = {"role": "user", "content": ""}
                messages_with_dummy = [dummy_user, message]
                inputs = processor.apply_chat_template(
                    messages_with_dummy,
                    tools=tools,
                    add_generation_prompt=False,
                ...
                # Calculate dummy prefix length to remove later
                dummy_inputs = processor.apply_chat_template(
                    [dummy_user],
                    tools=None,
                    add_generation_prompt=False,  # Add generation prompt to match what precedes assistant
                    tokenize=True,
                    return_dict=True,
                    return_tensors="pt",
                    **apply_chat_template_kwargs,
                )
                dummy_prefix_length = dummy_inputs["input_ids"].shape[1]
            else:
        ...
        # Remove dummy prefix if we added one
        if dummy_prefix_length > 0:
            input_ids = input_ids[dummy_prefix_length:]
            attention_mask = attention_mask[dummy_prefix_length:]
            # Also trim any other sequence-based tensors in inputs
            for key in list(inputs.keys()):
                if isinstance(inputs[key], torch.Tensor) and inputs[key].dim() >= 2:
                    # Trim sequence dimension (assumed to be dim 1)
                    inputs[key] = inputs[key][:, dummy_prefix_length:]
        # remove system prompt if exists (only if we didn't use dummy workaround)
        elif index != 0 and message["role"] != "system":
            # Only remove system prompt if we didn't add a dummy prefix
            # (because dummy conversations don't include the system prompt)
        ...
        if message["role"] == "assistant":
            ...
            # Skip this if we used dummy workaround (generation prompt already removed in that case)
            if dummy_prefix_length == 0:
