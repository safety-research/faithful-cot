...
class RayPPOTrainer:
    ...
    def _compute_old_log_prob(self, batch: DataProto):
        # CONFIGURABLE: Control whether old_log_prob (denominator in importance sampling) uses CoT masking
        # - If True: old_log_prob computed WITHOUT masking (even if actor uses masking)
        # - If False: old_log_prob uses same masking as actor
        # Allows independent control of numerator vs denominator in importance ratio
        disable_cot_masking_for_old_log_prob = self.config.actor_rollout_ref.actor.get(
            "disable_cot_masking_for_old_log_prob", False
        )
        ...
        if self.use_legacy_worker_impl == "disable":
            ...
            tu.assign_non_tensor(
                ...
                compute_loss=False,
                disable_cot_masking=disable_cot_masking_for_old_log_prob
            )
            output = self.actor_rollout_wg.compute_log_prob(batch_td)
            # gather output
            ...
            # For legacy implementation, set disable flag in meta_info
            batch.meta_info["disable_cot_masking"] = disable_cot_masking_for_old_log_prob
