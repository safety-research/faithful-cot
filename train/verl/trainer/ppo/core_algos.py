...
def compute_gae_advantage_return(
    ...
    with torch.no_grad():
        ...
        # Diagnostic: Check advantages before whitening
        if torch.isnan(advantages).any():
            logger.error(f"❌ NaN in advantages BEFORE whitening!")
            logger.error(f"   Rewards stats: min={token_level_rewards.min().item()}, max={token_level_rewards.max().item()}, mean={token_level_rewards.mean().item()}")
            logger.error(f"   Values stats: min={values.min().item()}, max={values.max().item()}, mean={values.mean().item()}")

        advantages = verl_F.masked_whiten(advantages, response_mask)

        # Diagnostic: Check advantages after whitening
        if torch.isnan(advantages).any():
            logger.error(f"❌ NaN in advantages AFTER whitening!")
