...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        # When CoT masking is enabled, we use SDPA which supports custom masks and is more stable than eager
        ...
        if use_cot_masking_actor or use_cot_masking_ref:
            if attn_implementation in ["flash_attention_2", "flash_attention_3"]:
                if self.rank == 0:
                    print(f"[CoT Masking] Forcing attn_implementation from '{attn_implementation}' to 'sdpa' "
                          f"because CoT masking requires custom attention masks (incompatible with Flash Attention)")
                    print(f"[CoT Masking] SDPA is more numerically stable than eager attention")
                attn_implementation = "sdpa"

        # CRITICAL: Attention gradient masking ONLY works with eager attention
        # The patch modifies eager_attention_forward, which is not used by Flash Attention or SDPA
        # OPTIMIZATION: Only force eager for ACTOR (which needs gradient masking during training)
        # Ref model can use faster implementations since it's frozen (no gradient masking needed)
        use_attention_gradient_masking_actor = self.config.actor.get("use_attention_gradient_masking", False) if hasattr(self.config, "actor") else False

        if use_attention_gradient_masking_actor:
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Gemma3ForConditionalGeneration (multimodal) has _no_split_modules with vision layers
            # that don't exist in the text-only model, causing FSDP wrap policy to fail.
            # We need to explicitly use Gemma3ForCausalLM instead of AutoModelForCausalLM.
            is_gemma3 = (
                hasattr(actor_model_config, "model_type") and actor_model_config.model_type == "gemma3_text"
            ) or (
                hasattr(actor_model_config, "architectures")
                and actor_model_config.architectures
                and "Gemma3" in actor_model_config.architectures[0]
            )
            ...
            if is_gemma3:
                ...
                if self.rank == 0:
                    print(f"Detected Gemma3 model, using Gemma3ForCausalLM explicitly")
            else:
            ...
            # Transfer CoT gradient masking config to model.config (after model loading, before monkey patching)
            import sys
            if role == "actor" and hasattr(self.config, 'actor'):
                use_cot_gradient_masking = self.config.actor.get('use_cot_gradient_masking', False)
                if self.rank == 0:
                    print(f"=" * 80, file=sys.stderr, flush=True)
                    print(f"DEBUG [fsdp_workers Rank 0]: Checking actor config for CoT gradient masking", file=sys.stderr, flush=True)
                    print(f"  - role: {role}", file=sys.stderr, flush=True)
                    print(f"  - use_cot_gradient_masking: {use_cot_gradient_masking}", file=sys.stderr, flush=True)
                if use_cot_gradient_masking:
                    actor_module.config.use_cot_gradient_masking = True
                    actor_module.config.end_think_token_id = self.config.actor.get('end_think_token_id', 151666)
                    actor_module.config.block_prompt_gradients = self.config.actor.get('block_prompt_gradients', True)
                    actor_module.config.block_answer_gradients = self.config.actor.get('block_answer_gradients', True)
                    if self.rank == 0:
                        print(f"  - ✓ Transferred to actor_module.config successfully", file=sys.stderr, flush=True)
                        print(f"=" * 80, file=sys.stderr, flush=True)

                # Transfer attention gradient masking config to model.config (independent from CoT parameter masking)
                use_attention_gradient_masking = self.config.actor.get('use_attention_gradient_masking', False)
                if self.rank == 0:
                    print(f"=" * 80, file=sys.stderr, flush=True)
                    print(f"DEBUG [fsdp_workers Rank 0]: Checking actor config for attention gradient masking", file=sys.stderr, flush=True)
                    print(f"  - role: {role}", file=sys.stderr, flush=True)
                    print(f"  - use_attention_gradient_masking: {use_attention_gradient_masking}", file=sys.stderr, flush=True)
                if use_attention_gradient_masking:
                    actor_module.config.use_attention_gradient_masking = True
                    actor_module.config.end_think_token_id = self.config.actor.get('end_think_token_id', 151666)
                    if self.rank == 0:
                        print(f"  - ✓ Transferred to actor_module.config successfully", file=sys.stderr, flush=True)
                        print(f"=" * 80, file=sys.stderr, flush=True)
