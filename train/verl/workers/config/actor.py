...
class ActorConfig(BaseConfig):
    ...
    Args:
        ...
        use_cot_masking (bool): Enable Chain-of-Thought structural masking during training.
            When enabled, answer tokens cannot attend to prompt tokens (must attend through CoT).
        think_token_id (int): Token ID for <think> tag (default: 151665).
        end_think_token_id (int): Token ID for </think> tag (default: 151666).
        disable_cot_masking_for_old_log_prob (bool): If True, disable CoT masking when computing
            old_log_prob for importance sampling. Allows independent control of numerator vs
            denominator masking in the importance ratio (default: False).
        use_cot_gradient_masking (bool): Enable CoT parameter gradient masking (Figure 3b).
            When enabled, only CoT tokens contribute to parameter updates.
        block_prompt_gradients (bool): Block gradients from prompt tokens to parameters.
        block_answer_gradients (bool): Block gradients from answer tokens to parameters.
        use_substring_delimiter_matching (bool): Use substring matching for delimiter detection.
            When False (default): uses special token mode with end_think_token_id.
            When True: uses substring matching with end_think_delimiter_str.
        end_think_delimiter_str (str): Delimiter string for substring matching mode (default: "</think>").
    """
    ...
    # CoT (Chain-of-Thought) masking configuration
    use_cot_masking: bool = False  # Enable structural CoT masking
    think_token_id: int = 151665  # Token ID for <think>
    end_think_token_id: int = 151666  # Token ID for </think>
    disable_cot_masking_for_old_log_prob: bool = False  # Disable masking for old_log_prob (importance sampling denominator)

    # CoT gradient masking configuration (Figure 3b intervention)
    # Blocks gradients from backpropagating from answer/prompt positions into model parameters
    use_cot_gradient_masking: bool = False  # Enable gradient masking for CoT
    block_prompt_gradients: bool = True  # Block gradients from prompt positions
    block_answer_gradients: bool = True  # Block gradients from answer positions
    use_substring_delimiter_matching: bool = False  # Use substring matching instead of special token
    end_think_delimiter_str: str = "</think>"  # Delimiter string for substring matching mode

    # Attention gradient masking configuration (answer→prompt gradient blocking)
    # Blocks gradients from flowing from answer tokens to prompt tokens through attention mechanism
    # This is independent from use_cot_gradient_masking and can be used alone
    use_attention_gradient_masking: bool = False  # Enable attention gradient masking

    # Prompt Embedding Perturbation (PEP / FACT) configuration
    # Perturbs prompt token hidden states at a chosen transformer layer during policy updates,
    # leaving CoT and answer positions untouched, forcing the model to route answer
    # prediction through the CoT rather than attending directly to prompt tokens.
    # PEP is automatically disabled during compute_log_prob (eval mode guard).
    use_prompt_embedding_perturbation: bool = False  # Enable PEP during policy update
    pep_epsilon: float = 0.05          # Perturbation magnitude (L2 norm for worst_case; fraction for random)
    pep_normalize: bool = True         # (random mode) scale noise by per-position hidden state L2 norm
    pep_layer: int = -1                # Layer to perturb: -1 = embed_tokens, N = model.layers[N]
    pep_mode: str = "worst_case"       # "random" (Gaussian) | "worst_case" (adversarial FGSM/PGD)
    pep_pgd_steps: int = 1             # PGD attack steps for worst_case (1 = FGSM)
    pep_dual_loss: bool = False        # If True: clean forward for PPO + perturbed forward as aux NLL loss
    pep_dual_loss_coef: float = 1.0    # Weight of the auxiliary perturbed NLL loss
    pep_attack_answer_only: bool = False  # If True: attack loss uses only answer tokens (after </think>)
