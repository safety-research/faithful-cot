...
OUTPUT_DIR = Path("/workspace-vast/jinghanj/workspace/Structural_RL_dev/data/cc-transformed-with-hints-v3")
...
PROMPT_TEMPLATE = """Fix the solve(lines) function below. First think step by step, then produce a corrected implementation that runs without exceptions and passes all provided assertions.

Do not modify or bypass the assertions.

Do not hardcode the provided examples or use a lookup table. The solution should generalize to unseen inputs.
