...
def create_tokenized_inputs(
    ...
    Args:
        ...
        hint_prefix: If provided (e.g. "A Stanford professor guessed"), narrow prompt_mask
                     to only cover tokens from the first occurrence of this substring in
                     the prompt onwards. This makes gradient/KL metrics measure only the
                     hint-specific portion of the prompt rather than the full context.
    ...
    # Pad to same length using RIGHT-padding.
    # With batch_size=1 (recommended) there is no padding at all, so padding side is irrelevant.
    actual_lengths = [ids.shape[0] for ids in input_ids_list]
    ...
    from verl.utils.cot_masking import find_special_token_positions, tokenize_delimiter_string
    ...
    if use_substring_matching:
        ...
        # tokenize_delimiter_string (from cot_masking.py) returns List[Tensor]:
        #   [base_ids, variant1_ids, ...] where variants handle Qwen-style merged boundaries
        #   e.g. ">" + "\n" → ">\n" (single token) in Qwen2.5 tokenizer.
        delimiter_str = "</think>"
        ...
        # DEBUG: Show all tokenized variants (handles Qwen merged-boundary tokens)
        print(f"[Eval] Substring matching mode: '{delimiter_str}'")
        for idx, variant in enumerate(end_delimiter_ids):
            decoded = tokenizer.decode(variant.tolist())
            print(f"  variant[{idx}]: {variant.tolist()} → {repr(decoded)}")
    else:
    ...
    # Zero out answer_mask for samples with no </think> so their metrics return NaN
    ...
    for i in range(batch_size):
        sample_input_ids = input_ids[i:i+1]  # [1, max_len]
        ...
        # If no </think> found: zero out answer_mask → all metrics return NaN
        if not cot_mask[i].any():
            answer_mask[i] = False
            n_no_cot += 1

    if n_no_cot > 0:
        print(f"[Eval] WARNING: {n_no_cot}/{batch_size} samples have no </think> → excluded from metrics (NaN)")

    # Optional: narrow prompt_mask to hint sentence only.
    # Finds the hint prefix in the prompt, then truncates at the first newline after it,
    # so prompt_mask covers only the hint sentence (e.g. " Stanford professor guessed 655.")
    # and not the format instructions that follow.
    if hint_prefix is not None:
        hint_prefix_ids = tokenize_delimiter_string(tokenizer, hint_prefix, device)[0]
        # Collect all sentence-ending / newline token variants.
        # Qwen2.5 merges period+newline into single tokens (e.g. ".\n\n" → 382, ".\n" → 624),
        # so we search for ".\n\n", ".\n", and "\n" variants and take the earliest match.
        sep_variants = (
            tokenize_delimiter_string(tokenizer, ".\n\n", device)
            + tokenize_delimiter_string(tokenizer, ".\n", device)
            + tokenize_delimiter_string(tokenizer, "\n", device)
        )
        n_hint_found = 0
        for i in range(batch_size):
            prompt_positions = prompt_mask[i].nonzero(as_tuple=True)[0]
            if len(prompt_positions) == 0:
                continue
            prompt_start = prompt_positions[0].item()
            prompt_end = prompt_positions[-1].item() + 1

            # Find hint sentence start
            pos = _find_first_subsequence(input_ids[i], hint_prefix_ids, start=prompt_start)
            if pos is None or pos >= prompt_end:
                print(f"[Eval] WARNING: sample {i} — hint_prefix not found in prompt, keeping full prompt_mask")
                continue

            # Find first sentence-ending separator after the hint prefix.
            # Check all variants (handles merged ".\n\n", ".\n", bare "\n") and take earliest.
            search_start = pos + len(hint_prefix_ids)
            sep_pos = None
            for sep_ids in sep_variants:
                p = _find_first_subsequence(input_ids[i], sep_ids, start=search_start)
                if p is not None and (sep_pos is None or p < sep_pos):
                    sep_pos = p

            # Zero out everything before the hint sentence
            prompt_mask[i, :pos] = False
            # Zero out everything from the separator onwards (hint sentence only, no format text)
            if sep_pos is not None and sep_pos < prompt_end:
                prompt_mask[i, sep_pos:] = False

            n_hint_found += 1

        print(f"[Eval] hint_prefix narrowing: found in {n_hint_found}/{batch_size} samples")
        print(f"[Eval] hint_prefix: {repr(hint_prefix)} → ids {hint_prefix_ids.tolist()}")
    ...
    if truncate_at_assert:
        ...
        # Use only the base variant (index 0) for the assert subsequence search
        assert_token_ids = tokenize_delimiter_string(tokenizer, "assert", device)[0]
        n_truncated = 0
        for i in range(batch_size):
    ...
    num_samples_with_delimiter = (cot_mask.any(dim=1)).sum().item()
